...
import org.rdfhdt.hdt.search.result.MapHDTQueryResult;
...
public class NestedJoinQueryIterator extends FetcherIterator<HDTQueryResult> {
		...
		for (HDTComponentTriple pattern : query.getPatterns()) {
			...
			if (sp.isVariable()) {
				...
				if (ivc == null) {
					...
					// replace by this constant, we can't update this variable because a previous value is
					// already defining it
					s = ivc.constant;
				}
			} else {
	...
	protected HDTQueryResult getNext() {
		...
		while (iterators.get(0) != null) {
			...
			for (int i = deep; i < iterators.size(); i++) {
				Iterator<TripleID> it = createPattern(i);
				if (!it.hasNext()) {
	...
	private Iterator<TripleID> createPattern(int id) {
