...
public class WriteDictionarySection implements DictionarySectionPrivate {
	...
	public WriteDictionarySection(HDTOptions spec, Path filename) {
	...
	public void load(TempDictionarySection other, ProgressListener plistener) {
		...
		try (CountOutputStream out = new CountOutputStream(tempFilename.openOutputStream(true))) {
			...
			for (Iterator<? extends CharSequence> it = other.getSortedEntries(); it.hasNext(); ) {
	...
	public void close() throws IOException {
		IOUtil.closeAll(tempFilename, blockTempFilename);
	}
}
