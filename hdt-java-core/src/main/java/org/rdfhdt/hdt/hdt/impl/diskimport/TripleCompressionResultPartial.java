...
public class TripleCompressionResultPartial implements TripleCompressionResult {
	...
	public TripleCompressionResultPartial(List<CloseSuppressPath> files, long tripleCount, TripleComponentOrder order) throws IOException {
	...
	private ExceptionIterator<TripleID, IOException> createBTree(List<CloseSuppressPath> files, int start, int end) throws IOException {
		int size = end - start;
		if (size <= 0) {
			return ExceptionIterator.empty();
		}
		if (size == 1) {
			CompressTripleReader r = new CompressTripleReader(files.get(start).openInputStream(true));
			this.files.add(r);
			return r;
		}
		int mid = (start + end) / 2;
