...
/**
 * Query tool factory, to register a new query tool, you need to add into the META-INF/services directory
 * a file named {@literal org.rdfhdt.hdt.search.HDTQueryToolFactory} with a line containing the name of the
 * class with the implementation. You have 2 different QueryTool, generic and not generic, respectively
 * get with {@link #newGenericQueryTool(HDT)} and {@link #newQueryTool(HDT)}. To say if you can create a
 * generic query tool, {@link #hasGenericTool()} should return true. If multiple generic tool are implemented,
 * the first implementation is used.
 * <p>
...
public abstract class HDTQueryToolFactory {
	...
	static List<HDTQueryToolFactory> getFactories() {
		...
		List<HDTQueryToolFactory> list = LOADER.stream()
				.map(ServiceLoader.Provider::get)
				.collect(Collectors.toList());
		if (list.isEmpty()) {
			throw new IllegalArgumentException("Can't find service for class " + HDTQueryToolFactory.class + "! Did you add the core to the libraries?");
		}
		return factory = list;
	}
	...
	/**
	 * create query tool from an HDT, this method should be specific to the implemented HDT and should not be generic,
	 * use {@link #newGenericQueryTool(HDT)} to create generic HDT query tool.
	 *
	 * @param hdt hdt
	...
	/**
	 * create a generic query tool from an HDT, this tool shouldn't be linked with the HDT implementation
	 *
	 * @param hdt hdt
	 * @return HDTQueryTool
	 */
